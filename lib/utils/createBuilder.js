...
module.exports = function (findBuilds, insertFile) {
    return function (options) {
        ...
        var getCallbackForPipeline = function (name) {
            return options.pipelines[name];
        };

        var pushStreamToPipeline = function (stream, name) {
            if ( options.pipelines[name] ) {
                return getCallbackForPipeline(name)(stream);
            }
            ...
            if ( _.isEmpty(processed.builds) ) {
                ...
                var streams = processed.builds.map(function (build) {
                    if ( build.files.length === 0 ) {
                        return getCallbackForPipeline(build.id)();
                    }

                    var stream = vfs.src(build.files, { cwd: options.assetsDir, base: options.assetsDir });
