...
var pipeIfStream = require("./pipeIfStream");
...
module.exports = function (findBuilds, insertFile) {
    return function (options) {
        ...
        // make new streams for each build
        _.each(options.pipelines, function (value, key) {
            pipelines[key] = value();
        });
        ...
        var process = function (file, encoding, callback) {
            ...
            if ( _.isEmpty(processed.builds) ) {
                ...
                var streams = processed.builds.map(function (build) {
                    ...
                    return pipeIfStream(stream, pipelines[build.id]);
                });
            ...
            pipeIfStream(mainPipeline, pipelines.main).on("data", function (file) {
                self.push(file);
                callback();
            });
        };
